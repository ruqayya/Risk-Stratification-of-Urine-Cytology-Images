function blur = blurMetric(original, heavyBlur)
...
B_Ver = round(imfilter(I,Hv));%blur the input image in vertical direction
B_Hor = round(imfilter(I,Hh));%blur the input image in horizontal direction
...
DFV = D_F_Ver(2:y-1,2:x-1);
DFH = D_F_Hor(2:y-1,2:x-1);

S_D_Ver = sum(DFV(:));
S_D_Hor = sum(DFH(:));

VV = V_Ver(2:y-1,2:x-1);
VH = V_Hor(2:y-1,2:x-1);
...
if heavyBlur
    ...
    n = 1;
    cutoff = floor(((x-2)*(y-2)*n)/100);

    S_D_VerA = sum(DFV(DFV > DFV(cutoff) & DFV < DFV(end-cutoff+1)));
    S_D_HorA = sum(DFH(DFH > DFH(cutoff) & DFH < DFH(end-cutoff+1)));

    S_V_VerA = sum(VV(VV > VV(cutoff) & VV < VV(end-cutoff+1)));
    S_V_HorA = sum(VH(VH > VH(cutoff) & VH < VH(end-cutoff+1)));

    blur_F_VerA = (S_D_VerA-S_V_VerA)/S_D_VerA;
    blur_F_HorA = (S_D_HorA-S_V_HorA)/S_D_HorA;

    blurA = max(blur_F_VerA,blur_F_HorA);

    %blur = (blur.*(min(1, blurA).^2)).^(1/3);
    blur = cat(3, blur, blurA);
end